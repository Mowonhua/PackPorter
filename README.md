...
- 目标文件不存在时初始化个人设置，无需先启动新整合包；只生成允许迁移的偏好和键位。已有空文件仍按合并处理。
- 只有读取返回 `NotFound` 才进入初始化；权限、编码等其他读取错误会使计划失败，不会作为空配置继续处理。
- `key_` 键位族：旧值优先；目标未列出的旧键位保留并计为“未验证键位”，不凭目标配置推断键位已淘汰。预览显示初始化或合并模式、设置总数与未验证键位数。
- 音量（`soundCategory_*`）、视角画质（`fov`/`gamma`/`guiScale`/`renderDistance`/`maxFps`）、语言等偏好键：同名采用旧值；旧版独有且值合法时补入新版。
- 新 Mod 新增键位：原样保留。
- 其余旧版遗留键（旧 Mod 残留）：智能忽略。
- 部分数值偏好有基础格式检查；未通过检查时保留目标已有值，目标缺少该键时不写入。此检查不保证取值范围或跨版本格式兼容。
- 未验证键位不保证目标游戏或 Mod 支持；跨 Minecraft 版本的键名和格式转换尚未实现。
